...
class PlayAction extends AbstractTextFocusRejectingAction {
    ...
    public void doActionPerformed(ActionEvent e) {
        try {
            ...
            Sequence thisSequence = noteSelection ==
                                    null ? musicSheet.getComposition().getSequence() : musicSheet.getComposition().getSelectedSequence(noteSelection.line, noteSelection.begin, noteSelection.end);

            if (MainFrame.sequencer.getTickPosition() >= MainFrame.sequencer.getTickLength() ||
                thisSequence != MainFrame.sequencer.getSequence()) {
            ...
            boolean playContinuously = playMenu.getMainFrame().getProperties().getProperty(Constants.PLAY_CONTINUOUSLY_PROP).equals(Constants.TRUE_VALUE);
            MainFrame.sequencer.setLoopCount(playContinuously ? Sequencer.LOOP_CONTINUOUSLY : 0);
            MainFrame.sequencer.start();

            if (!playContinuously) {
                playMenu.disableComponentsThread = new DisableComponentsThread(playMenu,
                        (MainFrame.sequencer.getMicrosecondLength() - MainFrame.sequencer.getMicrosecondPosition()) /
                        1000);
