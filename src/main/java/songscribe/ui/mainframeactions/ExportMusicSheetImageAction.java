...
public class ExportMusicSheetImageAction extends AbstractAction {
    ...
    private MyAcceptFilter[] myAcceptFilters = {
            new MyAcceptFilter("JPEG Bitmap", "jpg"), new MyAcceptFilter("GIF Image", "gif"),
            new MyAcceptFilter("Portable Network Graphics", "png"), new MyAcceptFilter("Windows Bitmap", "bmp")
    };
    ...
    public ExportMusicSheetImageAction(MainFrame mainFrame) {
        ...
        pfd = new PlatformFileDialog(mainFrame, "Export as Image", false, myAcceptFilters, Integer.parseInt(mainFrame.getProperties().getProperty(Constants.IMAGE_EXPORT_FILTER_PROP)));
    }
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (pfd.showDialog()) {
            ...
            mainFrame.getProperties().setProperty(Constants.IMAGE_EXPORT_FILTER_PROP, Integer.toString(Utilities.arrayIndexOf(myAcceptFilters, maf)));
            ...
            try {
                BufferedImage sheetImageForExport = mainFrame.getMusicSheet().createMusicSheetImageForExport(Color.white, scale, resolutionDialog.getBorder());
                ...
                if (!successful) {
                    mainFrame.showErrorMessage("Could not export the image file.\n" +
                                               "The " + extension +
                                               " image type might not be supported in your Java version.\n" +
