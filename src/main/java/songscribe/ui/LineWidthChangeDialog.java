...
public class LineWidthChangeDialog extends MyDialog implements ActionListener, ChangeListener {
    public static final double MIN_LINE_WIDTH = 2.0;
    public static final double MAX_LINE_WIDTH = 10.0;
    private JSlider lineWidthSlider = new JSlider((int) Math.round(
            MIN_LINE_WIDTH * MusicSheet.RESOLUTION), (int) Math.round(MAX_LINE_WIDTH * MusicSheet.RESOLUTION));
    ...
    public LineWidthChangeDialog(MainFrame mainFrame) {
        ...
        measureMents.setSelectedIndex(mainFrame.getProperties().getProperty(Constants.METRIC).equals(Constants.TRUE_VALUE) ? 0 : 1);
    ...
    protected void setData() {
        mainFrame.getProperties().setProperty(Constants.METRIC,
                measureMents.getSelectedIndex() == 0 ? Constants.TRUE_VALUE : Constants.FALSE_VALUE);
    }


    public void actionPerformed(ActionEvent e) {
        if (measureMents.getSelectedIndex() == 0) {
            lineWidthSpinner.setValue((double) mainFrame.getMusicSheet().getComposition().getLineWidth() /
                                      (double) MusicSheet.RESOLUTION * 2.54);
            lineWidthSpinner.setMinimum(MIN_LINE_WIDTH * 2.54);
            lineWidthSpinner.setMaximum(MAX_LINE_WIDTH * 2.54);
        }
        else {
            lineWidthSpinner.setValue((double) mainFrame.getMusicSheet().getComposition().getLineWidth() /
                                      (double) MusicSheet.RESOLUTION);
            lineWidthSpinner.setMinimum(MIN_LINE_WIDTH);
            lineWidthSpinner.setMaximum(MAX_LINE_WIDTH);
        }
    }
    ...
    public void stateChanged(ChangeEvent e) {
        if (stopChange) {
            ...
            mainFrame.getMusicSheet().setLineWidth((int) Math.round(
                    (Double) lineWidthSpinner.getValue() / (measureMents.getSelectedIndex() == 0 ? 2.54 : 1d) *
                    MusicSheet.RESOLUTION));
            stopChange = true;
