...
public class Book extends JComponent implements MouseListener, MouseMotionListener {
    private static final int PAGES_SPACE = 30;
    ...
    public Book(Publisher publisher, int pageWidth, int pageHeight, int leftInnerMargin, int rightOuterMargin, int topMargin, int bottomMargin, boolean mirroredMargin) {
        ...
        currentPageWriter = new ChangeListener() {
            public void stateChanged(ChangeEvent e) {
                Book.this.publisher.getPStatusBar().setCurrentPage(
                        getPage(scroll.getViewport().getViewPosition().y +
                                scroll.getViewport().getExtentSize().height / 2) + 1);
            }
        };
    ...
    public void setPageSize(int pageWidth, int pageHeight, int leftInnerMargin, int rightOuterMargin, int topMargin, int bottomMargin, boolean mirroredMargin) {
        ...
        oddMargin = new Rectangle(leftInnerMargin, topMargin,
                pageWidth - leftInnerMargin - rightOuterMargin, pageHeight - topMargin - bottomMargin);
        evenMargin = mirroredMargin ? new Rectangle(rightOuterMargin, topMargin,
                pageWidth - leftInnerMargin - rightOuterMargin, pageHeight - topMargin - bottomMargin) : oddMargin;
    }
    ...
    private int getPage(int y) {
        return Math.min(pages.size() - 1, Math.max(0, y / (int) (pageSize.height * scale + PAGES_SPACE)));
    }

    private int getPageY(int page, int y) {
        return y - page * (int) (pageSize.height * scale + PAGES_SPACE);
    }
    ...
    public double getNotGreaterResolution(double width, double height) {
        // we suppose oddMargin.width == evenMargin.width
    ...
    public void goToPage(int page) {
        ...
        viewport.setViewPosition(new Point(viewport.getViewPosition().x, Math.max(0, Math.min(
                getHeight() - viewport.getExtentSize().height, getScreenY(page - 1, 0)))));
        scroll.getViewport().addChangeListener(currentPageWriter);
    ...
    protected void paintComponent(Graphics g) {
        ...
        if (selectedComponent != null) {
            ...
            g2.drawRect(
                    0, getScreenY(selectedPage, 0),
                    (int) (pageSize.width * scale), (int) (pageSize.height * scale)
            );
        }

        if (dragging == Dragging.MOVING || dragging == Dragging.STRETCHING) {
            g2.setPaint(Color.black);
    ...
    private void paintStuff(Graphics2D g2, Rectangle view) {
        ...
        for (int i = firstPage; i <= lastPage; i++) {
            ...
            if (i == firstPage) {
                startY = getPageY(firstPage, view.y);
            }

            if (i == lastPage) {
                endY = getPageY(firstPage, view.y + view.height);
            }
    ...
    private void sizeChanged(Point newViewPort) {
        ...
        size.height = pages.size() * (int) (pageSize.height * scale) + (pages.size() - 1) * PAGES_SPACE + 10;
        setPreferredSize(size);
        revalidate();

        if (newViewPort != null) {
            scroll.getViewport().setViewPosition(newViewPort);
        }
    ...
    public void mousePressed(MouseEvent e) {
        ...
        selectedComponent = selection ==
                            Selection.COMPONENTS ? pages.get(selectedPage).findComponent(getAbsoluteCoordinates(e.getPoint(), selectedPage)) : null;
    ...
    public void mouseDragged(MouseEvent e) {
        ...
        if (dragging == Dragging.MOVING) {
            ...
            selectionRect.x = Math.min(getRelativeX(margin.width, page) -
                                       selectionRect.width, Math.max(getRelativeX(0, page), selectionRect.x));
            repaint();
        }
    }
    ...
    private enum Dragging { NONE, SELECTED, MOVING, STRETCHING }
