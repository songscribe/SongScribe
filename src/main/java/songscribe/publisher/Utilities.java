...
public class Utilities {
    ...
    public static double convertFromPixels(int pixel, int mode) {
        return mode == 0 ?
                Math.round((double) pixel / MusicSheet.RESOLUTION * 100.0) / 100.0 :
                Math.round((double) pixel / MusicSheet.RESOLUTION * 25.4);
    }
    ...
    public static int convertToPixel(double length, int mode) {
        return mode == 0 ?
                (int) Math.round(length * MusicSheet.RESOLUTION) :
                (int) Math.round(length / 25.4 * MusicSheet.RESOLUTION);
    }
}
