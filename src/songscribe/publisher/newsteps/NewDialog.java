...
public class NewDialog extends JDialog{
    ...
    public NewDialog(Publisher publisher) {
        ...
        pack();
        setLocation(Publisher.CENTERPOINT.x-getWidth()/2, Publisher.CENTERPOINT.y-getHeight()/2);
        setVisible(true);
        setStep(publisher.getProperties().getProperty(Constants.SHOWPUBLISHERNEWINFO).equals(Constants.TRUEVALUE) ? 0 : 1);
    }
    ...
    private void setStep(int step){
        ...
        nextButton.setAction( step==steps.length-1 || step==steps.length-2 && data.files.size()==0  ? new GenerateAction() : new NextAction());
        steps[step].start();
    }
    ...
    private class GenerateAction extends AbstractAction implements Runnable{
        ...
        public void run() {
            ...
            for(int i=0;i<data.files.size();i++){
                ...
                if(i==data.files.size()-1 || currentHeight+heights[i+1]>drawableHeight || i-firstInThisPage+1==data.songsPerPage){//page break
                    int space = (drawableHeight-currentHeight)/(i==firstInThisPage ? 1 : i-firstInThisPage);
                    int currentY = 0;
                    for(int j=firstInThisPage;j<=i;j++){
                        int xPos = (int)Math.round((drawableWidth-musicSheets[j].getSheetWidth()*resolutions[j])/2.0);
                    ...
                    firstInThisPage = i+1;
                    currentHeight = 0;
                    if(i<data.files.size()-1)page = book.addPage();
                }
                processDialog.nextValue();
            }
